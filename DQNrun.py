...
def sliding_window(target, labels, window_size=10, stripe=1):
    # get sliced sequenceses where window_size=40, stripe/moving_step=1
    x = []
    y = []
...
def train(train_flag, batch_size, l_action, k_state, Episodes):
    ...
    # load datasets and model depending on whether training or testing
    if train_flag:
        # adt dataset, note that training set is any data that contains both normal and abnormal
        # e.g. for adt dataset, we extract data[1500:2000]
        score_ae = np.load("datasets//ae_score.npy")[1500:2000]
        print(min(score_ae),max(score_ae))
        y_labels = np.load("datasets//windows_attack_labels.npy")[1500:2000]
        #print(score_ae.shape, y_labels.shape)
    ...
    if not train_flag:
        ...
        print("max", max(score_ae), "min", min(score_ae))
        y_labels = np.load("datasets//windows_attack_labels.npy")
        print("testing size:", len(score_ae), "anomaly count:", sum(y_labels), "max score:", np.max(score_ae),
              "min score:", np.min(score_ae))

        # load the trained model
        agent = DQNagent(state_size, action_size)
        env = Env(action_set, y_labels, score_ae)
        agent.q_net = agent.target_net = keras.models.load_model("saved_model")
        epsilon = 0
    ...
    for e in range(0, Episodes):
        ...
        if not train_flag:  # testing result
            ...
            print("Testing Result==> precision:{} recall:{} F1: {} testing time:{}".format(precision, recall, f1_score, time.time() - start_time))
            print("TP:{} TN:{} FP:{} FN:{}".format(env.tp_n, env.tn_n, env.fp_n, env.fn_n))
            #np.save("datasets//binary_threshold.npy", np.array(binary_threshold))


if __name__ == '__main__':
    # train
    train(train_flag=True, batch_size=256, l_action=10, k_state=1, Episodes=2000)

    # test
    #train(train_flag=False, batch_size=256, l_action=1, k_state=1, Episodes=1)
